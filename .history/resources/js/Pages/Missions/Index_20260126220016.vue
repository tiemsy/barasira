...
async function loadMissions() {
    ...
    result.data.forEach(mission => {
        document.getElementById('missions').innerHTML += `
            <li id="mission-${mission.id}">
                <strong>${ mission.title }</strong>
                <span class="status-badge ${mission.status}"> Status :  ${ mission.status }</span>
                <span> Date : ${ mission.date_start }</span>
            </li>
        `
    })
    ...
    if (result.next_page_url) {
        ...
        document.getElementById('loader').innerText = "Fin des missions";
    }
    // missions.value = res.data
}

window.addEventListener('scroll', () => { if (window.innerHeight + window.scrollY >= document.body.offsetHeight - 200) { loadMissions(); } });
